...
parser.add_argument("--nl", type=float, help="noise level", default=0.1)
parser.add_argument("--lr-VAE", type=float, help="learning rate for VAE Training", default=0.0001)
parser.add_argument("--lr-IFNO", type=float, help="learning rate for IFNO Training", default=0.005)
parser.add_argument(
    "--lr-forward",
    type=float,
    help="learning rate for joint Training-forward",
    default=0.0001,
)
parser.add_argument(
    "--lr-backward",
    type=float,
    help="learning rate for joint Training-backward",
    default=0.0001,
)
...
parser.add_argument("--n-train", type=int, help="num of train dataset", default=400)
parser.add_argument("--n-valid", type=int, help="num of valid dataset", default=100)
parser.add_argument("--n-test", type=int, help="num of test dataset", default=100)
...
parser.add_argument("--batchsize3", type=int, help="num of batchsize for testing", default=10)
...
y_no_noise = np.load("./data/wave_Z_u.npy")[:, ::10, ::2]
x_no_noise = np.load("./data/wave_Z_k.npy")[:, ::2, ::2]

logger.info("    data shape, f mean, u mean")
logger.info("   " + str(x_no_noise.shape) + " " + str(x_no_noise.mean()) + " " + str(y_no_noise.mean()))

if args.nl == 0.0:
    y = np.load("./data/wave_Z_u.npy")[:, ::10, ::2]
    x = np.load("./data/wave_Z_k.npy")[:, ::2, ::2]
    extra_f = np.load("./data/wave_Z_k.npy")[:ntrain, ::2, ::2]
elif args.nl == 0.1:
    y = np.load("./data/wave_Z_u_01.npy")[:, ::10, ::2]
    x = np.load("./data/wave_Z_k_01.npy")[:, ::2, ::2]
    extra_f = np.load("./data/wave_Z_k_01.npy")[:ntrain, ::2, ::2]
elif args.nl == 0.2:
    y = np.load("./data/wave_Z_u_02.npy")[:, ::10, ::2]
    x = np.load("./data/wave_Z_k_02.npy")[:, ::2, ::2]
    extra_f = np.load("./data/wave_Z_k_02.npy")[:ntrain, ::2, ::2]
...
def train(ifno=1):
    ...
    def pretrain():
        ...
        if ifno:
            ...
            optimizer2 = torch.optim.AdamW(model.parameters(), lr=0.005)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.5, verbose=True)
            min_err_forward = 1.0
            ...
            for ep in tqdm(range(epochs_IFNO)):
                ...
                for x, y in train_loader:
                    ...
                    pred_y = pred_y.reshape(batchS, mm, s, 3)
                ...
                with torch.no_grad():
                    for x, y, x_no_noise, y_no_noise in test_loader:
                        ...
                        pred_y = pred_y.reshape(batchS, mm, s, 3)
                        pred_y = y_normalizer.decode(pred_y.clone())
                        y_true = y_normalizer.decode(y_no_noise.reshape(batchS, mm, s, 3).clone())
                        pred_x, _ = model.backward(y.reshape(batchS, mm, s, 3))
                        pred_x = x_normalizer.decode(pred_x.reshape(batchS, s, s, 3).clone())
    ...
    def joint_train():
        ...
        with torch.no_grad():
            ...
            if 1 == 1:
                for x, y, x_no_noise, y_no_noise in test_loader:
                    ...
                    mse_test += (
                        (pred_x[:, :, :, 0].reshape(batchS, -1) - x_true[:, :, :, 0].reshape(batchS, -1)) ** 2
                    ).mean()
        for ep in tqdm(range(epochs)):
            ...
            train_l2_backward /= ntrain
            logger.info(
                f"  Epoch [{ep + 1}/{epochs}], Training Loss Forward: {train_l2_forward:.4f}, Training Loss Backward: {train_l2_backward:.4f}, Test Loss Forward: {test_l2_forward:.4f}, Test Loss Backward: {test_l2_backward:.4f}, Min. Test Forward Error: {min_err_forward:.4f}, Min. Test Backward Error: {min_err_backward:.4f}, MSE: {mse_test:.4f}"
            )
