...
parser.add_argument("--lr-VAE", type=float, help="learning rate for VAE Training", default=0.0001)
parser.add_argument("--lr-IFNO", type=float, help="learning rate for IFNO Training", default=0.0005)
parser.add_argument("--lr-forward", type=float, help="learning rate for joint Training-forward", default=0.00005)
parser.add_argument("--lr-backward", type=float, help="learning rate for joint Training-backward", default=0.00005)
...
parser.add_argument("--n-train", type=int, help="num of train dataset", default=1000)
parser.add_argument("--n-valid", type=int, help="num of valid dataset", default=100)
parser.add_argument("--n-test", type=int, help="num of test dataset", default=200)
parser.add_argument("--batchsize", type=int, help="num of batchsize for training", default=10)
parser.add_argument("--batchsize2", type=int, help="num of batchsize for vae training", default=200)
parser.add_argument("--batchsize3", type=int, help="num of batchsize for testing", default=50)
parser.add_argument("--batchsize4", type=int, help="num of batchsize for joint training", default=50)
...
batch_size4 = args.batchsize4
epochs = args.epochs
beta = args.beta


y_no_noise = np.load("./data/NS_u.npy").reshape(1200, 128, 128)[:, ::2, ::2]
x_no_noise = np.load("./data/NS_k.npy").reshape(1200, 128, 128)[:, ::2, ::2]
...
if args.nl == 0.0:
    ...
    y = np.load("./data/NS_u.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    x = np.load("./data/NS_k.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    extra_f = np.load("./data/NS_k.npy").reshape(1200, 128, 128)[:ntrain, ::2, ::2]
elif args.nl == 0.1:
    logger.info("   load 0.1 noise data")
    y = np.load("./data/NS_u_01.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    x = np.load("./data/NS_k_01.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    extra_f = np.load("./data/NS_k_01.npy").reshape(1200, 128, 128)[:ntrain, ::2, ::2]
    epochs_IFNO = 100
    epochs = 10
elif args.nl == 0.2:
    y = np.load("./data/NS_u_02.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    x = np.load("./data/NS_k_02.npy").reshape(1200, 128, 128)[:, ::2, ::2]
    extra_f = np.load("./data/NS_k_02.npy").reshape(1200, 128, 128)[:ntrain, ::2, ::2]
    epochs_IFNO = 65
...
train_loader3 = torch.utils.data.DataLoader(
    torch.utils.data.TensorDataset(xtr, ytr), batch_size=batch_size4, shuffle=True
)
        ...
        for ep in tqdm(range(epochs_VAE)):
            ...
            with torch.no_grad():
                for x, y, x_no_noise, y_no_noise in test_loader:
                    ...
                    diff = unnorm_recon_img[:, :, :, 0].reshape(batchS, -1) - unnorm_img[:, :, :, 0].reshape(batchS, -1)
                    test_l2 += (
        ...
        for ep in tqdm(range(epochs_IFNO)):
            ...
            with torch.no_grad():
                for x, y, x_no_noise, y_no_noise in test_loader:
                    ...
                    test_grid += (
                        ((diff_forward_grid**2).sum(1) / ((y_true[:, :, :, 1:].reshape(batchS, -1) ** 2).sum(1))) ** 0.5
                    ).sum()
                    test_grid += (
    ...
    def joint_train():
        ...
        for ep in tqdm(range(epochs)):
            ...
            for x, y in train_loader3:
                ...
                pred_y = y_normalizer.decode(pred_y.clone())
                y_true = y_normalizer.decode(y.clone())
                loss_forward = myloss(pred_y.view(batchS, -1), y_true.view(batchS, -1))
            ...
            if test_l2_backward < min_err_backward:
                min_err_backward = test_l2_backward
            logger.info(
                f"  Epoch [{ep + 1}/{epochs}], Training Loss Forward: {train_l2_forward:.4f}, Training Loss Backward: {train_l2_backward:.4f}, Test Loss Forward: {test_l2_forward:.4f}, Test Loss Backward: {test_l2_backward:.4f}, Min. Test Forward Error: {min_err_forward:.4f}, Min. Test Backward Error: {min_err_backward:.4f}"
            )
